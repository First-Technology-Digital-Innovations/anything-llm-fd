...
const CORPORATE_STYLES_XML = (() => {
  ...
  try {
    const xml = fs.readFileSync(stylesPath, "utf-8");
    console.log(
      `[create-docx-file] Loaded corporate styles from ${stylesPath} (${xml.length} chars)`
    );
    return xml;
  } catch (e) {
    console.warn(
      `[create-docx-file] Could not load corporate styles from ${stylesPath}: ${e.message}. Falling back to default docx styles.`
    );
    return null;
  }
})();
          ...
          handler: async function ({
            ...
            try {
              ...
              const doc = new Document({
                ...
                ...(CORPORATE_STYLES_XML
                  ? { externalStyles: CORPORATE_STYLES_XML }
                  : {}),
                sections,
              });
